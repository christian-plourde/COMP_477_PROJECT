...
void Pac::Draw() 
{
	if (!isDead)
	{
		if (invulnerable)
		{
			pacmanHitTimer += Time::deltaTime;
			if (pacmanHitTimer >= pacmanHitCooldown)
			{
				pacmanHitTimer = 0;
				invulnerable = false;
			}
...
bool Pac::ConsumeMap(int x, int y)
{
	...
	if (GameBoard::initial_map[idx][x] == 0 || GameBoard::initial_map[idx][x] == 5)
	{
		...
		//SoundManager::Instance()->playSound(SoundEffects::Pacman_Chomp);

		return true;
	} else if (GameBoard::initial_map[idx][x] == 3)
	{
		// consume energizer
		GameBoard::initial_map[idx][x] = -1;
		SoundManager::Instance()->playSound(SoundEffects::Pacman_PowerUp);
